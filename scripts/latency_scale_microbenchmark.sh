...
do
  ...
  do
    ...
    do
      ...
      do
        output_file="$RACK_FILL-$batch_size-$num_machines-$num_cores-$num_memslices-$num_processes-$rack_util-$iter.log"
        cmd="java -cp $JAR_PATH $MAIN_CLASS -a $batch_size -c $num_cores -f $RACK_FILL -m $num_memslices -n $num_machines -p $num_processes -u $rack_util"
        echo "Fill: $RACK_FILL, Batch: $batch_size, ClusterConfig: (${num_machines} ${num_cores} ${num_memslices}) Util: $rack_util Iter: $iter OutputFile: $output_file"
        echo $cmd
